...
const AGENT_QA_PRIMER = [
  ...
  '',
  'SIGN-IN — do it YOURSELF, before any navigation that might need auth:',
  ...
  '- Sign the chosen persona into THIS chat\'s browser by POSTing its id:',
  '    curl -s -X POST "$AGENT_QA_BASE/api/chat/c/$AGENT_QA_CHAT_ID/connect" \\',
  '      -H "content-type: application/json" -d \'{"personaId":"<id>"}\'',
  '  It resolves credentials + runs the auth plugin, returning {"authenticated":true,…}',
  '  once signed in (can take ~30-60s). Connect BEFORE the first navigation — the',
  '  sign-in cookie is per-session and does NOT carry over from an earlier turn. If a',
  '  snapshot later shows a sign-in page, connect again and retry.',
  '',
  'RECORDING an authenticated flow:',
  '  (1) Connect the persona (above) — ideally before navigating, but connecting',
  '      at ANY point during the recording binds that profile to the scenario so it',
  '      RE-AUTHENTICATES on replay (the workbench sets it as the recording baseline',
  '      and as $AGENT_QA_PROFILE, so a plain `agent-qa start "<intent>"` already',
  '      records under that login — you do NOT need to pass --profile yourself).',
  '  (2) `agent-qa start "<intent>"` (no --session, no --profile needed).',
  '  (3) `agent-browser --session "$AGENT_BROWSER_SESSION" snapshot` to CONFIRM you are',
  '      PAST the sign-in screen. If you still see a "Sign in" heading, connect again',
  '      and retry — do NOT proceed.',
  '  (4) Only THEN record the navigation step + the rest. NEVER record or assert the',
  '      sign-in page as if it were the target — that means auth failed: stop and fix',
