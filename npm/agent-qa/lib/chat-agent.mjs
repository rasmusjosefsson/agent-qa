...
const AGENT_QA_PRIMER = [
  ...
  '  (3) Re-open the target URL (connect leaves the tab elsewhere), then',
  '      `agent-browser --session "$AGENT_BROWSER_SESSION" snapshot` to CONFIRM you are',
  '      PAST the sign-in screen. If you still see a "Sign in" heading AFTER re-opening,',
  '      auth genuinely failed — re-read the connect `log`, fix the cause, connect again;',
  '      do NOT proceed and do NOT ask the user to sign in.',
