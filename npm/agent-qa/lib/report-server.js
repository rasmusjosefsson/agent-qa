'use strict';
// agent-qa report viewer — L2.
//
// A localhost-only, read-only web view over L0's per-run sidecar tree.
...
// there is no new Rust crate (see tmp/dashboard-plan.md → L2).
//
...
// Editor (L3) trigger kinds, mirrors recorder_shape::TriggerKind. The
...
// -------- editor (L3) — write surface via the Rust CLI --------
//
// The authoring editor needs a *write* path (start a recording session,
// record/run/reorder steps, flush a scenario.json). Per the L3 plan, all
// writes go through the existing Rust plumbing — never to a network
...
function createRequestHandler(root, deps) {
  return async function handle(req, res) {
    try {
      ...
      // Editor (L3) write surface. Only mounted when a CLI runner is
