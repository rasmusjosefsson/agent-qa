...
await runAutomationExerciseGolden("tc24", "Automation Exercise TC24 download invoice after purchase", async (golden) => {
  const unique = Date.now();
  await golden.openUrl("https://www.automationexercise.com/login", "open signup login page");
  await golden.fillSelector('input[data-qa="signup-name"]', `Agent QA ${unique}`, "enter signup name");
  ...
  await golden.fillSelector('input[data-qa="mobile_number"]', "555010024", "enter mobile number");
  ...
  await golden.fillSelector('#ordermsg textarea[name="message"]', "TC24 invoice comment", "enter order comment");
