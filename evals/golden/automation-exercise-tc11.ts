...
await runAutomationExerciseGolden("tc11", "Automation Exercise TC11 subscription on cart page", async (golden) => {
  const email = `agent-qa-cart-subscribe-${Date.now()}@example.com`;
  await golden.openHome();
  await golden.waitSelectorText("#slider", "AutomationExercise", "home page is visible");
  await golden.domClickSelector('a[href="/view_cart"]', "open cart page");
