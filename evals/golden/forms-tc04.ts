...
const runId = `golden-forms-tc04-${new Date().toISOString().replace(/[:.]/g, "-")}`;
...
const session = `golden-forms-tc04-${Math.random().toString(16).slice(2, 8)}`;
...
async function main(): Promise<void> {
  ...
  let error = "";
  try {
    const start = await run("start", [agentQa, "start", "Forms TC04 invalid phone validation", "--session", session]);
