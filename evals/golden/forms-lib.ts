...
const formsUrl = "https://qaplayground.com/practice/forms";
...
export interface FormsGolden extends GoldenContext {
...
function createContext(tc: string, intent: string): GoldenContext {
  const runId = `golden-forms-${tc}-${new Date().toISOString().replace(/[:.]/g, "-")}`;
  ...
  const session = `golden-forms-${tc}-${Math.random().toString(16).slice(2, 8)}`;
...
export async function runFormsGolden(
  tc: string,
  intent: string,
  steps: (golden: FormsGolden) => Promise<void>,
): Promise<void> {
  ...
  const golden: FormsGolden = {
    ...ctx,
    async openPage() {
      await run(ctx, "open forms", [ctx.agentBrowser, "--session", ctx.session, "open", formsUrl]);
      // Practice content hydrates client-side — wait for the first form card.
      await run(ctx, "wait practice content", [ctx.agentBrowser, "--session", ctx.session, "wait", '[data-testid="form-login-inner"]']);
      await record(ctx, "navigation", { route: formsUrl });
      await record(ctx, "wait", { condition: { kind: "selector", selector: '[data-testid="form-login-inner"]' }, intent: "forms page rendered" });
    },
